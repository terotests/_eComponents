    ...
    (function (_myTrait_) {
      ...
      _myTrait_.bsComps = function (body) {
        ...
        */
        ...
        body.customElement("bsProgressbar", {
          meta: {
            category: "Indicators"
          },
          data: {
        ...
        body.customElement("bsPagination", {
          meta: {
            category: "Navigation"
          },
          data: {
        ...
        body.customElement("dropdownMenu", {
        ...
        body.customElement("dropdownButton", {
          meta: {
            category: "Buttons"
          },
          data: {
          ...
          init: function init(data) {
            ...
            popper = this.div().e("dropdownMenu");
        ...
        var create_bs_btn = function create_bs_btn(scope, type) {
          var ucFirst = type.charAt(0).toUpperCase() + type.slice(1);
          scope.customElement("btn" + ucFirst, {
            meta: {
              category: "Buttons"
            },
            data: {
      ...
      _myTrait_.materialComps = function (body) {
        ...
        paper_input(body, "paperInput", "#4a89dc");
        paper_input(body, "paperTextarea", "#4a89dc", "textarea");
        ...
        paper_circle(body, "paperCircle", "200px", "rgba(255, 255, 255, 0.25)");
        paper_circle(body, "paperCircle", "200px", "rgba(255, 0, 0, 0.25)");
        paper_circle(body, "paperCircleGreen", "200px", "rgba(0, 255, 0, 0.25)");
        paper_circle(body, "paperCircleDark", "200px", "rgba(0, 0, 0, 0.15)");
        paper_circle(body, "paperCircleHuge", "1200px", "rgba(255, 255, 255, 0.12)", 1.4);
        ...
        create_btn(body, "paperButton", "gray", "0px");
        create_btn(body, "priButton", "gray", "0px");
        create_btn(body, "dangerButton", "red", "0px");
        create_btn(body, "warningButton", "yellow", "0px");
        ...
        alert_info(body, "alertInfo", "#4a89dc");
        ...
        paper_heading(body, "paperH1", "2em", "#333");
        paper_heading(body, "paperH2", "1.5em", "#666");
        paper_heading(body, "paperH3", "1.3em", "#666");
        ...
        send_email_comp(body, "sendEmail");
        ...
        support_question(body, "supportQuestion");
      ...
      _myTrait_.ownComps = function (t) {
        _e().createClass("dataTable", {
          ...
          init: function init() {
            ...
            // analyze the rows
            if (model && model.isArray && model.isArray()) {

              if (this.props().get("insertData")) {
                this.button("btn btn-success btn-sm").text(this.props().get("insertTitle") || "+ item ").on("click", function () {
                  model.push(JSON.parse(this.props().get("insertData")));
                });
              }

              var tbl = _e("table");
              tbl.width("100%");
              // insert headings for the columns
              // dataTable
              // -> rows or items to be displayed on the table...
              // -> how to get those items
              var hNames;
              if (hNames = this.props().get("headings")) {
                var hlist = hNames.split(",");
                var trow = _e("tr");
                hlist.forEach(function (n) {
                  var th = _e("th");
                  th.text(n);
                  trow.add(th);
                });
              ...
              var propList = this.props().get("props").split(",");
              var propData = this.props();

              var tBody = _e("tbody");
              tBody.mvc(model, function (item) {
                var tr = _e("tr");
                propList.forEach(function (propName) {
                  var td = _e("td");
                  var pn = propName.trim();

                  var propItem = item;
                  var subParts = pn.split(".");
                  if (subParts.length > 1) {
                    propName = subParts.pop().trim();
                    while (subParts.length) {
                      var subProp = subParts.shift();
                      propItem = propItem[subProp];
                    }
                  }

                  td.input("form-control").width("100%").bind(propItem, propName.trim());
                  // td.span().text(JSON.stringify( propItem.toData())+"::"+propName);
                  tr.add(td);
                });
                var btnGroup;
                if (propData.get("actions")) {
                  btnGroup = _e("div");
                  btnGroup.addClass("btn-group");
                  var aList = propData.get("actions").split(",");
                  aList.forEach(function (actionStr) {
                    var parts = actionStr.split("=>");
                    btnGroup.button("btn btn-sm btn-default").text(parts[0]).clickTo(parts[1].trim(), item);
                  });
                }
                if (propData.get("deleteTitle")) {
                  if (!btnGroup) {
                    btnGroup = _e("div");
                    btnGroup.addClass("btn-group");
                  }
                  btnGroup.button("btn btn-sm btn-default").text(propData.get("deleteTitle")).on("click", function () {
                    if (confirm(propData.get("deleteTitle") + "?")) item.remove();
                  });
                }
                if (btnGroup) {
                  var td = _e("td");
        ...
        // Any message type seems to be possible...
        _e().createClass("Kurkkari", {
          meta: {
            category: "Alerts"
          },
          data: {
          ...
          show: function show() {
            if (this._isVisibleIn) return;
            this._isVisibleIn = true;
            this._isHidden = false;
            var me = this;
            var dir = this.props().get("direction");
            var ease = this.props().get("easing");
            var box = this.box();
            var w = box.width,
                h = box.height;

            if (dir == "left") {
              later().ease(ease, 500, function (t) {
                me.x(-1 * w * (1 - t));
              }, function () {
                me.send("is_visible", me);
              });
            }
            if (dir == "right") {
              me._dom.style.left = "auto";
              later().ease(ease, 500, function (t) {
                me._dom.style.right = -1 * w * (1 - t) + "px";
              }, function () {
                me.send("is_visible", me);
              });
            }
            if (dir == "top") {
              ...
              me._dom.style.left = this.state().get("x");
              later().ease(ease, 500, function (t) {
                me._dom.style.top = -1 * h * (1 - t) + "px";
              }, function () {
                me.send("is_visible", me);
              });
            }
            if (dir == "bottom") {
              me._dom.style.top = "auto";
              later().ease(ease, 500, function (t) {
                me._dom.style.bottom = -1 * h * (1 - t) + "px";
              }, function () {
                me.send("is_visible", me);
              });
            }
          ...
          hide: function hide() {
            ...
            this._isHidden = true;
            this._isVisibleIn = false;

            var me = this,
                w = this.state().get("width");
            var ease = this.props().get("easing");
            var box = this.box();
            var w = box.width,
                h = box.height;
            ...
            if (dir == "left") {
              later().ease(ease, 500, function (t) {
                ...
                me.send("is_hidden", me);
              });
            }
            if (dir == "right") {
              later().ease(ease, 500, function (t) {
                me._dom.style.right = -1 * w * t + "px";
              }, function () {
                me.send("is_hidden", me);
              });
            }
          ...
          init: function init() {
            ...
            var me = this;
            setTimeout(function () {
              me.width(model.get("width"));
              me.height(model.get("height"));

              var box = me.box();
              w = box.width;

              var dir = me.props().get("direction") || "left";
        ...
        _e().createClass("CKEdit", {
          ...
          init: function init() {
            ...
            if (!this.state().get("html") && _lsName && typeof localStorage != "undefined") {
        ...
        _e().createClass("aceEditor", {
          ...
          init: function init() {
            ...
            var varName = this.props().get("varName") || "code";
            var dObj = this.model();

            if (!dObj.hasOwn(varName) && _lsName && typeof localStorage != "undefined") {
              this.state().set(varName, localStorage[_lsName] || "");
            }
            editor.setValue(this.model().get(varName));
